...
defaults.trainSteps   = '250000'
defaults.retrainSteps = '250000'
...
workflow explore {
    ...
    setup = inputs.map{[it[1]+[meta:it[0]], getParams(defaults, it[1])]}
        .map{[it[0]+[seeds:it[1].trainSeeds.toInteger()], it[1]]}
    ...
    qbcInps    = traj.flatMap{(1..it[0].seeds).collect(seed->[it[0]+[seed:seed], it[1]])}
        .join(models.filter{it[0].seed!=1}).map{[it[0], [ds:it[1], inp:it[2], subDir:"trajs/iter${it[0].iter}/qbclabel${it[0].seed}"]]}
    qbcLabels  = traj.mix(qbcLabel(qbcInps)).map{meta,inp -> [groupKey(meta+[seed:1],meta.seeds), inp]}.groupTuple()
    toLabel    = qbcFilter(qbcLabels.map{[it[0], [ds:it[1], subDir:"trajs/iter${it[0].iter}/qbcfilter"]]})
    labelInp2  = labelInp.join(toLabel).map{[it[0], it[1]+[ds:it[2], subDir:"trajs/iter${it[0].iter}/label"]]}
